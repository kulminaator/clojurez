...
result=$($TOOL_TIMEOUT $TIMEOUT $VM /tmp/cljvm_test_lazy_no_overeval.clj 2>&1) || {
    echo "FAIL: file-exec: lazy-seq no over-evaluation [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
if [ "$result" = "count=4 result=6" ]; then
    echo "PASS: file-exec: lazy-seq no over-evaluation [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: file-exec: lazy-seq no over-evaluation [$( _elapsed)]"
...
result=$($TOOL_TIMEOUT $TIMEOUT $VM /tmp/cljvm_test_script_no_eval_output.clj 2>&1) || {
    echo "FAIL: file-exec: no eval output from def/defn [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
if [ "$result" = "sum=30 product=200" ]; then
    echo "PASS: file-exec: no eval output from def/defn [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: file-exec: no eval output from def/defn [$( _elapsed)]"
...
LONG_EXPR_4K="(+ $(seq 1 3000 | sed 's/.*/1/' | paste -sd' ' -))"
...
result=$($TOOL_TIMEOUT $TIMEOUT bash -c "printf '%s\n(println :ok)\n(exit)\n' '$LONG_EXPR_4K' | $VM --repl 2>&1" ) || {
    echo "FAIL: repl long expression >4096 bytes [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
...
if echo "$result" | grep -q "3000" && echo "$result" | grep -q ":ok"; then
    echo "PASS: repl long expression >4096 bytes [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: repl long expression >4096 bytes [$( _elapsed)]"
...
LONG_EXPR_8K="(+ $(seq 1 6000 | sed 's/.*/1/' | paste -sd' ' -))"
...
result=$($TOOL_TIMEOUT $TIMEOUT bash -c "printf '%s\n(println :ok)\n(exit)\n' '$LONG_EXPR_8K' | $VM --repl 2>&1" ) || {
    echo "FAIL: repl long expression >8192 bytes [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
if echo "$result" | grep -q "6000" && echo "$result" | grep -q ":ok"; then
    echo "PASS: repl long expression >8192 bytes [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: repl long expression >8192 bytes [$( _elapsed)]"
...
result=$($TOOL_TIMEOUT $TIMEOUT bash -c "printf '(ns mytest)\nundefined-symbol-xyz\n(println :done)\n(exit)\n' | $VM --repl 2>&1" ) || {
    ...
    if [ $? -eq 124 ]; then
        echo "FAIL: ns parent cycle: undefined symbol caused hang [$( _elapsed)] (cycle not fixed)"
    else
        echo "FAIL: ns parent cycle: timeout or error [$( _elapsed)]"
    fi
    TEST_FAIL=$((TEST_FAIL + 1))
}
...
if echo "$result" | grep -qi "UndefinedSymbol\|Error" && echo "$result" | grep -q ":done"; then
    echo "PASS: ns parent cycle: undefined symbol gives error, no hang [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: ns parent cycle: expected error + :done in output [$( _elapsed)]"
    echo "  Got:      $(echo "$result" | tr '\n' ' ' | head -c 300)"
    TEST_FAIL=$((TEST_FAIL + 1))
fi
...
") || {
    echo "FAIL: refer selective [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
...
if echo "$result" | grep -q ":alpha" && echo "$result" | grep -q ":beta" && echo "$result" | grep -qi "UndefinedSymbol"; then
    echo "PASS: refer selective [:refer [alpha]] [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: refer selective [:refer [alpha]] [$( _elapsed)]"
    echo "  Got: $(echo "$result" | tr '\n' ' ' | head -c 300)"
    TEST_FAIL=$((TEST_FAIL + 1))
fi
...
") || {
    echo "FAIL: refer :all [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
if echo "$result" | grep -q ":x" && echo "$result" | grep -q ":y"; then
    echo "PASS: refer :all [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: refer :all [$( _elapsed)]"
    echo "  Got: $(echo "$result" | tr '\n' ' ' | head -c 300)"
    TEST_FAIL=$((TEST_FAIL + 1))
fi
...
") || {
    echo "FAIL: refer :all :exclude [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
# a should work, b should error (excluded)
if echo "$result" | grep -q ":a" && echo "$result" | grep -qi "UndefinedSymbol"; then
    echo "PASS: refer :all :exclude [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: refer :all :exclude [$( _elapsed)]"
    echo "  Got: $(echo "$result" | tr '\n' ' ' | head -c 300)"
    TEST_FAIL=$((TEST_FAIL + 1))
fi
...
") || {
    echo "FAIL: refer + as combined [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
if echo "$result" | grep -q "hi-1" && echo "$result" | grep -q "bye-2"; then
    echo "PASS: refer + as combined [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: refer + as combined [$( _elapsed)]"
    echo "  Got: $(echo "$result" | tr '\n' ' ' | head -c 300)"
    TEST_FAIL=$((TEST_FAIL + 1))
fi
...
result=$(CLJVM_DEBUG=1 $TOOL_TIMEOUT $TIMEOUT $VM -e '(+ 1 2)' 2>&1) || {
    echo "FAIL: debug output CLJVM_DEBUG=1 [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
if echo "$result" | grep -q "clojurez starting" && echo "$result" | grep -q "clojurez shutting down"; then
    echo "PASS: debug output CLJVM_DEBUG=1 shows startup/shutdown [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: debug output CLJVM_DEBUG=1 missing startup/shutdown [$( _elapsed)]"
    echo "  Got: $(echo "$result" | tr '\n' ' ' | head -c 300)"
    TEST_FAIL=$((TEST_FAIL + 1))
fi
...
result=$(CLJVM_DEBUG=startup $TOOL_TIMEOUT $TIMEOUT $VM -e '(+ 1 2)' 2>&1) || {
    echo "FAIL: debug output CLJVM_DEBUG=startup [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
...
if [ "$startup_count" -ge 2 ]; then
    echo "PASS: debug output CLJVM_DEBUG=startup shows startup category [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: debug output CLJVM_DEBUG=startup missing messages [$( _elapsed)]"
    echo "  Got: $(echo "$result" | tr '\n' ' ' | head -c 300)"
    TEST_FAIL=$((TEST_FAIL + 1))
fi
...
result=$(CLJVM_DEBUG=gc $TOOL_TIMEOUT $TIMEOUT $VM -e '(+ 1 2)' 2>&1) || {
    echo "FAIL: debug output CLJVM_DEBUG=gc [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
if ! echo "$result" | grep -q "clojurez starting"; then
    echo "PASS: debug output CLJVM_DEBUG=gc filters out startup category [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: debug output CLJVM_DEBUG=gc should not show startup messages [$( _elapsed)]"
    echo "  Got: $(echo "$result" | tr '\n' ' ' | head -c 300)"
    TEST_FAIL=$((TEST_FAIL + 1))
fi
...
result=$(unset CLJVM_DEBUG && $TOOL_TIMEOUT $TIMEOUT $VM -e '(+ 1 2)' 2>&1) || {
    echo "FAIL: debug output no env var [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
if ! echo "$result" | grep -q "clojurez starting"; then
    echo "PASS: debug output disabled by default (no CLJVM_DEBUG) [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: debug output should be disabled without CLJVM_DEBUG [$( _elapsed)]"
    echo "  Got: $(echo "$result" | tr '\n' ' ' | head -c 300)"
    TEST_FAIL=$((TEST_FAIL + 1))
fi
...
result=$(CLJVM_DEBUG=1 $TOOL_TIMEOUT $TIMEOUT $VM -e '(doall (map (fn [x] (* x 2)) (list 1 2 3)))' 2>&1) || {
    echo "FAIL: debug output CLJVM_DEBUG=1 crash regression [$( _elapsed)] (timeout or error)"
    TEST_FAIL=$((TEST_FAIL + 1))
}
if echo "$result" | grep -q "clojurez starting"; then
    echo "PASS: debug output CLJVM_DEBUG=1 no crash on map (regression) [$( _elapsed)]"
    TEST_PASS=$((TEST_PASS + 1))
else
    echo "FAIL: debug output CLJVM_DEBUG=1 crash regression [$( _elapsed)]"
    echo "  Got: $(echo "$result" | tr '\n' ' ' | head -c 300)"
    TEST_FAIL=$((TEST_FAIL + 1))
fi
