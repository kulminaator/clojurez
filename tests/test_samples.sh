...
hanoi_result=$($TOOL_TIMEOUT $TIMEOUT $VM tests/complex-samples/sample_2_hanoi/hanoi/core.clj 2>&1) || {
...
TEST_TOTAL=$((TEST_TOTAL + 1))
fib_result=$($TOOL_TIMEOUT $TIMEOUT bash -c "$VM tests/complex-samples/sample_1_fibonacci/core.clj 2>&1 | tail -1") || {
...
TEST_TOTAL=$((TEST_TOTAL + 1))
ns_result=$($TOOL_TIMEOUT $TIMEOUT $VM -cp tests/complex-samples/sample_3_namespaces/src -m main 2>&1) || {
...
TEST_TOTAL=$((TEST_TOTAL + 1))
gc_result=$($TOOL_TIMEOUT $TIMEOUT bash -c "$VM tests/complex-samples/sample_4_gc_stress/core.clj 2>&1 | tail -1") || {
