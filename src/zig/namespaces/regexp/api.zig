...
pub fn core_re_pattern(self: *Value, args: list.List, env: *Env) anyerror!Value {
    ...
    // Build pattern map: { :pattern "..." }
    var map = Value.Map.empty;
    errdefer map.deinit(allocator);

    var pattern_kw = try Value.keywordValue(allocator, "pattern");
    defer pattern_kw.deinit(allocator);
    var pattern_val = try Value.stringValue(allocator, s);
    defer pattern_val.deinit(allocator);

    // Clone the values into the map (clone duplicates the internal strings)
    try map.append(allocator, .{
        .key = try pattern_kw.clone(allocator),
        .value = try pattern_val.clone(allocator),
    });
