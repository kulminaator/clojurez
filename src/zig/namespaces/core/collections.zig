...
pub fn core_conj(self: *const Value, args: *const list.List, env_env: *Env) anyerror!Value {
    ...
    switch (std.meta.activeTag(coll)) {
        ...
        .list => {
            ...
            while (true) {
                try new_list.append(env_env.allocator, try vm.clone(&args.items[i], env_env.allocator));
                if (i == 1) break;
                i -= 1;
            }
            for (coll.list.items.items) |item| {
                try new_list.append(env_env.allocator, try vm.clone(&item, env_env.allocator));
            }
        ...
        .queue => {
            ...
            for (coll.queue.items.items) |item| {
                try new_queue.append(env_env.allocator, try vm.clone(&item, env_env.allocator));
            }
            for (args.items[1..]) |item| {
                try new_queue.append(env_env.allocator, try vm.clone(&item, env_env.allocator));
            }
        ...
        .map => {
            ...
            for (coll.map.entries.items) |entry| {
                try new_map.append(env_env.allocator, .{
                    .key = try vm.clone(&entry.key, env_env.allocator),
                    .value = try vm.clone(&entry.value, env_env.allocator),
                });
            }
            for (args.items[1..]) |item| {
                ...
                while (j < new_map.items.len) : (j += 1) {
                    if (vm.equals(new_map.items[j].key, entry_items[0])) {
                        ...
                        new_map.items[j].value = try vm.clone(&entry_items[1], env_env.allocator);
                        found = true;
                        break;
                    }
                }
                if (!found) {
                    try new_map.append(env_env.allocator, .{
                        .key = try vm.clone(&entry_items[0], env_env.allocator),
                        .value = try vm.clone(&entry_items[1], env_env.allocator),
                    });
                }
            }
        ...
        .record => {
            ...
            for (args.items[1..]) |item| {
                ...
                const current_clone = try vm.clone(&current, allocator);
                var assoc_args: list.List = .empty;
                defer assoc_args.deinit(allocator);
                try assoc_args.append(allocator, current_clone);
                try assoc_args.append(allocator, try vm.clone(&entry_items[0], allocator));
                try assoc_args.append(allocator, try vm.clone(&entry_items[1], allocator));
...
pub fn core_pop(self: *const Value, args: *const list.List, env_env: *Env) anyerror!Value {
    ...
    switch (std.meta.activeTag(coll)) {
        .vector => {
            ...
            var i: usize = 0;
            while (i < len) : (i += 1) {
                try new_vec.append(env_env.allocator, try vm.clone(&coll.vector.items.items[i], env_env.allocator));
            }
            return try vm.vectorValue(env_env.allocator, new_vec);
        },
        .list => {
            ...
            var i: usize = 0;
            while (i < len) : (i += 1) {
                try new_list.append(env_env.allocator, try vm.clone(&coll.list.items.items[i], env_env.allocator));
            }
            return try vm.listValue(env_env.allocator, new_list);
        },
        .queue => {
            ...
            while (i < coll.queue.items.items.len) : (i += 1) {
                try new_queue.append(env_env.allocator, try vm.clone(&coll.queue.items.items[i], env_env.allocator));
            }
            return try vm.queueValue(env_env.allocator, new_queue);
        },
        else => return error.TypeError,
...
pub fn core_last(self: *const Value, args: *const list.List, env_env: *Env) anyerror!Value {
    ...
    if (args.items.len != 1) return error.ArityError;
    const allocator = env_env.allocator;
    var coll = try vm.clone(&args.items[0], allocator);
    ...
    switch (std.meta.activeTag(coll)) {
        .vector => {
            ...
            return try vm.clone(&coll.vector.items.items[coll.vector.items.items.len - 1], allocator);
        },
        .list => {
            if (coll.list.items.items.len == 0) return vm.nilValue();
            return try vm.clone(&coll.list.items.items[coll.list.items.items.len - 1], allocator);
        },
        .string => {
            const s = coll.string;
...
pub fn core_reverse(self: *const Value, args: *const list.List, env_env: *Env) anyerror!Value {
    ...
    switch (std.meta.activeTag(coll)) {
        ...
        .lazy_seq => {
            ...
            while (i > 0) {
                i -= 1;
                try new_list.append(env_env.allocator, try vm.clone(&forced.list.items.items[i], env_env.allocator));
            }
            return try vm.listValue(env_env.allocator, new_list);
        },
        else => return error.TypeError,
...
pub fn core_peek(self: *const Value, args: *const list.List, env_env: *Env) anyerror!Value {
    ...
    switch (std.meta.activeTag(coll)) {
        .queue => {
            ...
            return try vm.clone(&coll.queue.items.items[0], env_env.allocator);
        },
        .vector => {
            if (coll.vector.items.items.len == 0) return vm.nilValue();
            return try vm.clone(&coll.vector.items.items[coll.vector.items.items.len - 1], env_env.allocator);
        },
        .list => {
            if (coll.list.items.items.len == 0) return vm.nilValue();
            return try vm.clone(&coll.list.items.items[coll.list.items.items.len - 1], env_env.allocator);
        },
        else => return error.TypeError,
    }
}
