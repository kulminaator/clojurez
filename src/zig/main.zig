...
const test_clj = @import("namespaces/core/test_clj.zig");
...
pub fn main(init: std.process.Init.Minimal) anyerror!void {
    ...
    // Create clojure.test namespace — testing framework.
    // Parent set to clojure.core so test code can use core functions.
    const ctest_env = try ns_mgr.createNamespace("clojure.test");
    ctest_env.parent = clojure_core_env;
    ctest_env.ns_manager = ns_mgr;

    // Load embedded Clojure test library into clojure.test namespace.
    try loadTestLibrary(allocator, ctest_env);
...
/// Load the embedded Clojure test library silently (no output for defn names).
fn loadTestLibrary(allocator: Allocator, env: *Env) anyerror!void {
    const content = test_clj.test_clj_source;
    ...
    for (forms.items) |form| {
        ...
        vm.valueDeinit(&result_ptr.*, allocator);
        // GC handles result cleanup.
        // Silent: don't print results during test library loading
    }
}
