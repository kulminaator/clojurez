...
pub fn evalThreadLast(allocator: Allocator, forms: []const Value, frame: *vm.Frame, depth: usize) anyerror!Value {
    ...
    const first_ptr_r = try eval.evalRec(allocator, &forms[0], frame, depth);
...
pub fn evalCondThreadFirst(allocator: Allocator, forms: []const Value, frame: *vm.Frame, depth: usize) anyerror!Value {
    ...
    while (i + 1 < forms.len) : (i += 2) {
        const test_ptr_r = try eval.evalRec(allocator, &forms[i], frame, depth);
    const test_ptr = test_ptr_r.value;
        if (vm.isTruthy(test_ptr.*)) {
            ...
            if (std.meta.activeTag(step.*) == .list and step.*.list.items.items.len > 0) {
                ...
                const op_ptr_r = try eval.evalRec(allocator, &step.*.list.items.items[0], frame, depth);
