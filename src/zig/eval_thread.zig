...
pub fn evalThreadLast(allocator: Allocator, forms: []const Value, frame: *vm.Frame, depth: usize) anyerror!Value {
    // Disable trampolining: threading macros must evaluate synchronously.
    // trampoline_allowed is thread-local so this is safe.
    ...
    const first_ptr = try eval.evalRecV(allocator, &forms[0], frame, depth);
    ...
    while (i < forms.len) : (i += 1) {
        ...
        if (std.meta.activeTag(form.*) != .list) {
            ...
            const next_ptr = try eval.evalRecV(allocator, &call_list, frame, depth);
...
pub fn evalThreadFirst(allocator: Allocator, forms: []const Value, frame: *vm.Frame, depth: usize) anyerror!Value {
    // Disable trampolining: threading macros must evaluate synchronously.
    const saved = eval.trampoline_allowed;
    eval.trampoline_allowed = false;
    defer eval.trampoline_allowed = saved;
...
pub fn evalCondThreadFirst(allocator: Allocator, forms: []const Value, frame: *vm.Frame, depth: usize) anyerror!Value {
    // Disable trampolining: threading macros must evaluate synchronously.
    const saved = eval.trampoline_allowed;
    eval.trampoline_allowed = false;
    defer eval.trampoline_allowed = saved;
    ...
    while (i + 1 < forms.len) : (i += 2) {
        const test_ptr = try eval.evalRecV(allocator, &forms[i], frame, depth);
        if (vm.isTruthy(test_ptr.*)) {
            ...
            if (std.meta.activeTag(step.*) == .list and step.*.list.items.items.len > 0) {
                ...
                const op_ptr = try eval.evalRecV(allocator, &step.*.list.items.items[0], frame, depth);
                ...
                while (j < step.*.list.items.items.len) : (j += 1) {
                    const arg_ptr = try eval.evalRecV(allocator, &step.*.list.items.items[j], frame, depth);
...
// (cond->> expr test1 step1 test2 step2 ...)
pub fn evalCondThreadLast(allocator: Allocator, forms: []const Value, frame: *vm.Frame, depth: usize) anyerror!Value {
    // Disable trampolining: threading macros must evaluate synchronously.
    const saved = eval.trampoline_allowed;
    eval.trampoline_allowed = false;
    defer eval.trampoline_allowed = saved;
