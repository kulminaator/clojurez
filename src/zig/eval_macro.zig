...
pub fn unquoteProcess(allocator: Allocator, form: Value, frame: *vm.Frame, depth: usize, ctx: ?*eval.TrampolineStack) anyerror!Value {
    switch (std.meta.activeTag(form)) {
        .list => {
            ...
            if (std.meta.activeTag(first) == .symbol) {
                if (std.mem.eql(u8, first.symbol, "unquote")) {
                    ...
                    const r = try eval.evalRec(allocator, &form.list.items.items[1], frame, depth, ctx);
                    const result = r.value.*;
                    return result;
                }
                if (std.mem.eql(u8, first.symbol, "unquote-splicing")) {
                    if (form.list.items.items.len != 2) return error.ArityError;
                    const result_r = try eval.evalRec(allocator, &form.list.items.items[1], frame, depth, ctx);
            ...
            for (form.list.items.items) |item| {
                ...
                if (std.meta.activeTag(item) == .list and item.list.items.items.len == 2) {
                    ...
                    if (std.meta.activeTag(uq_first) == .symbol and std.mem.eql(u8, uq_first.symbol, "unquote-splicing")) {
                        const splice_r = try eval.evalRec(allocator, &item.list.items.items[1], frame, depth, ctx);
                ...
                const processed = try unquoteProcess(allocator, item, frame, depth, ctx);
        ...
        .vector => {
            ...
            for (form.vector.items.items) |item| {
                try result.append(allocator, try unquoteProcess(allocator, item, frame, depth, ctx));
            }
